...
Time-stamp: <2018-07-07 13:36:08 tony>

* Current Status: IMPROVING

  Ok, hacking again for the first time in 4 years :(.
...
#+end_src
     ...
     CURRENT: Looks like it is solved? 
...
** Common Lisp
   ...
   implementation such as SBCL (though CCL or CLISP might work) and
   BLAS/LAPACK on your system.

   SBCL (not this year, yet: CCL) are in active use by developers.  We
...
*** Linux
    ...
    Most distributions contain SBCL (or CLISP, but please send
    fixes/instructions for CLISP or CCL if needed!)
...
#+END_SRC
    ...
    For Linux, IF BLAS IS INSTALLED IN A WEIRD PLACE, you need to make
    sure the loader looks for it.
...
#+end_src
    ...
    (CCL, CMUCL, and CLISP may work, but not tested)
...
prime question: at what point do we abstract data structures into the higher level 
relational domain-specific structures?  

Classic example:  dataframe/datasheet "rows/cols" as "obsn/vars".  

underlying structure (matrix, list-of-list, array)  could be managed through appropriate structures -- 
however, we really want to have a notion of the statistical data concepts in the code (variables, 
observations, correlations, etc.
