...
(defun lm.fit (x y (offset nil) (method 'qr) (tol 1e-07) (singular.ok t) &varargs)
  
  (if (not  (typep x 'matrix)) (error "x must by a matrix."))
  (if (= (nrow-non-na x) 0) (error "need >0 non-NA cases (data)"))
  (if (= (ncol-non-na x) 0) (error "need >0 non-NA vars (model)"))
