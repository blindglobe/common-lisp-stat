...
;;; Time-stamp: <2014-02-26 12:47:56 blindglobe@gmail.com>
;;; Creation:   <2014-02-26 11:09:24 blindglobe@gmail.com>
;;; File:       modelframe.lisp
;;; Author:     AJ Rossini <blindglobe@gmail.com>
;;; Copyright:  (c)2008--2010, AJ Rossini.  See LICENSE.mit in
;;;             toplevel directory for conditions.  

;;; Purpose: This is a second attempt at reproducing some of the most
;;;          important features of R's dataframe structure and
;;;          features, using a virtual CLOS-based class.  This file
;;;          currently contains the virtual class MODELFRAME-LIKE.  The
;;;          actual classes MODELFRAME-ARRAY and MODELFRAME-MATRIXLIKE
...
;;; xarray and slice APIs.  All other code should be either in the
;;; storage-specific files, or in the combined functions files.

(in-package #:cls-modelframe)

;;; When possible, we want to place statistical semantics into the
;;; computational data object!

;;; For example, we should computationally state that it is a
;;; violation of use to consider rows which are not at the least
;;; conditionally independent (though the conditioning should be
;;; outside the data set, not internally specified), though the
;;; assumption violation should not be set up so that we can never
;;; violate -- only be reminded that we are indeed violating it, under
;;; the principle that statistical assumptions are made to be
;;; violated.
...
;;; abstract class: modelframe-like

;; A modelframe-like class is an abstract class which looks like a
...
;;
;; 7. We need to move the matrix-like views and permutations here as
;;    reasonable? 
;;
...
(defclass list-of-types ()
  ((typelist :initform nil
	     :initarg typelist
	     :type list
	     :accessor typelist)
   (documentation :initform ""
		  :initarg doc
		  :type string
		  :accessor doc)))


(defun make-list-of-one-type (type-to-init &optional (docstring "none."))
  (is-type check-type type-to-init type)
  (make-instance list-of-types :typelist  (list type-to-init) :doc docstring))

(defun return-modelframe-types (modelframe-1)
  (map 'list-of-types 'get-supertype-of-list 
       (column 1 modelframe-1)
       (column 2 modelframe-1)
       ...
       (column M modelframe-1)))




(defclass modelframe-like ()
  (
