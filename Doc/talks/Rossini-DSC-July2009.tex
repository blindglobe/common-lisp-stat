...
\begin{frame}{What does NOT work?}
  ...
  (which is the subject of another talk, slides in the backup).
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
  \item Iterant : R/LispStat like system for Clozure (Lisp on the JVM)
  ...
  \end{itemize}
  Finally: support for a new statistical programming environment
  modality (subject for another talk).
\end{frame}
...
\begin{frame}{Can we compute with them?}
  3 Examples, of which we only present the first
  \begin{itemize}
  \item Research.
  ...
  (that is:
  \begin{itemize}
  \item do we have sufficient information to communicate enough
    for the right person to recreate the analysis?
...
\begin{frame}{Comments}
  ...
  \item I'm working on some basic statistical proof of concepts (not
    finished): linear regression (LS-based, Normal-bayesian) and the
    T-test.
